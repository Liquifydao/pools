...
import { USDT_STAKING_ABI } from "../../../config/contracts2";

export default function WithdrawFormUsdt({ contractAddress, onWithdrawSuccess }) {
  ...
  const loadStakedBalance = async () => {
    ...
    try {
      ...
      const formattedStaked = formatUnits(stakedAmount, 6); // USDT = 6 decimals
  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex justify-between items-center mb-2">
        <p className="text-gray-400 text-sm">Your Staked USDT</p>
        <p className="text-sm font-semibold text-white">
          {parseFloat(stakedBalance).toFixed(2)} USDT
        </p>
      </div>
